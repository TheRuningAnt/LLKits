...
- (void)setDayModel:(DayModel *)dayModel{
    ...
    if (_dayModel.isInMonth) {
        ...
        self.inter_Calendar.text = @"";
    }
    
    if (_dayModel.showSignTip) {
    
        UIImageView *tipImageV = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"signTip"]];
        tipImageV.contentMode = UIViewContentModeCenter;
        CGFloat w = CGRectGetWidth(self.bounds);
        CGFloat h = CGRectGetHeight(self.bounds);
        
        tipImageV.center = CGPointMake(w/2, h/2);
        tipImageV.tag = 1001;
        CGFloat signView_w = w > h ? h:w;
        tipImageV.bounds = CGRectMake(0, 0, signView_w, signView_w);
        [self addSubview:tipImageV];
    }else{
        
        UIView *tipV = [self viewWithTag:1001];
        if (tipV) {
...
-(void)clicpBorderWithIndex:(NSInteger)index{
    
    UIRectCorner rectType = UIRectCornerBottomLeft;
    if (index == 34 || index == 41) {
        
        rectType = UIRectCornerBottomRight;
    }
    
    UIBezierPath * path = [UIBezierPath bezierPathWithRoundedRect:self.layer.bounds byRoundingCorners:rectType cornerRadii:CGSizeMake(5, 5)];
...
- (void)layoutSubviews{
    ...
    CGFloat signView_w = w > h ? h:w;
    self.signView.bounds = CGRectMake(0, 0, w, h);
...
- (void)setSignDay:(BOOL)signDay{
    ...
    if (_signDay) {
        ...
        self.signView.backgroundColor = [UIColor cyanColor];
    }
}
