...
- (void)calendarContainerWhereMonth:(NSInteger)currentTag signDays:(NSArray*)signDays month:(void(^)(MonthModel *month))currentMonth{
    ...
    [[MonthOpeartion defaultMonthOpeartion] calendarContainerWithNearByMonths:currentTag daysOfMonth:^(NSArray *daysArray,MonthModel *month) {
        ...
        if (daysArray.count>0) {
            ...
            for (DayModel *model in daysArray) {
                ...
                for (NSString *day in signDays) {
                    if (model.month == month.month && model.day == [day integerValue]) {
                        
                        model.showSignTip = YES;
                        break;
                    }
                }
            }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.daysArray.count < 42) {
        if (indexPath .row == 28 || indexPath.row == 34) {
            
            [cell clicpBorderWithIndex:indexPath.row];
        }
    }else{
        
        if (indexPath .row == 35 || indexPath.row == 41) {
